...
class TrajComp():
    ...
    def step(self, episode, action, index, done, label='T'):        
        ...
        if LAST_P > self.link_head:
            ...
            self.err_record[(self.B_ward[LAST_P][1], NEXT_P)] = F.speed_op(self.ori_traj_set[episode][self.B_ward[LAST_P][1]: NEXT_P + 1])
        ...
        if NEXT_P < self.link_tail:
            ...
            self.err_record[(LAST_P, self.F_ward[NEXT_P][1])] = F.speed_op(self.ori_traj_set[episode][LAST_P: self.F_ward[NEXT_P][1] + 1])
